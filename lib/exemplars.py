...
class ExemplarHandler(nn.Module, metaclass=abc.ABCMeta):
    ...
    def reduce_exemplar_sets(self, m):
        ...
        for y, P_y in enumerate(self.exemplar_sets):
            n_max = len(P_y)
            indeces_selected = np.random.choice(n_max, size=min(m,n_max),replace=False)
            self.exemplar_sets[y] = [P_y[i] for i in indeces_selected]


    def construct_exemplar_set(self, dataset, n,class_id=None,task=1):
        ...
        if self.select_method == "herding":
            ...
            for k in range(min(n, n_max)):
                ...
                index_selected = np.argmin(torch.norm(features_dists.detach().cpu(), p=2, dim=1))
            ...
            for k in list_of_selected:
                self.exemplar_sets[class_id].append(dataset[k][0].numpy())
            ...
            for k in indeces_selected:
                self.exemplar_sets[class_id].append(dataset[k][0].numpy())
        elif self.select_method == "entropy":
            first_entry = True
            dataloader = utils.get_data_loader(dataset, 128, cuda=self._is_on_cuda(),shuffle=False)
            n_max=len(dataset)
            for (image_batch,_) in dataloader:
                image_batch = image_batch.to(self._device())
                with torch.no_grad():
                ...
                if first_entry:
                    ...
                    outputs = torch.cat([outputs, output_batch], dim=0)
            p_o = F.softmax(outputs,dim=1)
            e_p = -p_o*torch.log(p_o)
            e_p = torch.sum(e_p,dim=1)
            index_selected = torch.argsort(e_p,descending=True)[:min(n,n_max)]
            for k in index_selected:
                self.exemplar_sets[class_id].append(dataset[k][0].numpy())
        elif self.select_method == "distance":
            first_entry = True
            dataloader = utils.get_data_loader(dataset, 128, cuda=self._is_on_cuda(), shuffle=False)
            n_max = len(dataset)
            for (image_batch, _) in dataloader:
                ...
                if first_entry:
                    ...
                    outputs = torch.cat([outputs, output_batch], dim=0)
            distance = abs(outputs[:,class_id])
            index_selected = torch.argsort(distance, descending=False)[:min(n, n_max)]
            for k in index_selected:
                self.exemplar_sets[class_id].append(dataset[k][0].numpy())
        ...
        for i,e in enumerate(self.exemplar_sets):
            print(i,len(e))
