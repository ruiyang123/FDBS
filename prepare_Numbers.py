...
def get_dataset_info(source):
    ...
    df.to_csv("./Numbers_info.csv", index=False)
...
if __name__ == "__main__":
    source = r"C:\Users\ruiya\Numbers"
    # get_dataset_info(source)

    # DIL
    Numbers_task_order = {"MNIST": 1, "MNIST_M": 2, "SVHN": 3, "SYN": 4}
    info_path = "./Numbers_info.csv"
    Numbers_info = pd.read_csv(info_path)
    df = simple_domain_split(Numbers_info, task_order=Numbers_task_order)
    df.to_csv("./Numbers_DIL.csv")

    # smooth-DIL

    df = domain_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothDIL1.csv")

    df = domain_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothDIL2.csv")

    df = domain_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothDIL3.csv")

    # CIL
    df = class_split(df=Numbers_info, task_num=10, mode="class_inc")
    df.to_csv("./Numbers_CIL.csv")

    # smooth-CIL
    df = class_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothCIL1.csv")

    df = class_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothCIL2.csv")

    df = class_split(df=Numbers_info, task_num=6, mode="random")
    df.to_csv("./Numbers_smoothCIL3.csv")
